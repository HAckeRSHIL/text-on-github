...
OWNER = "HAckeRSHIL"
EMAIL = "47114974+HAckeRSHIL@users.noreply.github.com"
...
def cleanup_repo(
    new_branch_name="temp-branch", commit_message="Initial commit", force_push=False
):
...
def get_contributions():
    ...
    response = requests.post(
        "https://api.github.com/graphql", headers=headers, json=body
    )
...
def find_highest_contribution(contributions):
    ...
    for week in contributions["data"]["user"]["contributionsCollection"][
        "contributionCalendar"
    ]["weeks"]:
...
def create_local_repo():
    ...
    run_command(
        f"git remote set-url origin https://{TOKEN}@github.com/{OWNER}/{REPO_NAME}.git"
    )
...
def create_remote_repo(delete_existing=False):
    ...
    try:
        response = requests.post(
            CREATE_ENDPOINT, headers=headers, data=json.dumps(data)
        )
        response.raise_for_status()
...
def main():
    ...
    commit_dates_dark = get_commit_dates(
        printing_matrix, sunday_at_start + timedelta(weeks=SKIP_WEEKS_FROM_FRONT)
    )
    commit_dates_shadow = get_commit_dates(
        printing_matrix, sunday_at_start + timedelta(weeks=SKIP_WEEKS_FROM_FRONT - 1)
    )
